...
HAL_StatusTypeDef r_value=HAL_OK;
...
{      
    ...
    { 
        RTC_Set_Time(23,59,56,RTC_HOURFORMAT12_PM);	        //设置时间 ,根据实际时间修改,小时,分钟,秒钟
				RTC_Set_Date(20,03,25,3);		                    //设置日期,年，月，日，星期
...
{ 
	...
	r_value = HAL_RTCEx_SetWakeUpTimer_IT(&RTC_Handler,cnt,wksel);            //设置重装载值和时钟 
