...
#ifndef _BSP_LTC6804_H__
#define _BSP_LTC6804_H__
...
#define BYTES_IN_REG 	(8)		
#define REG_BYTES		(6)		
#define CMD_LEN			(4)		
#define CELL_IN_REG		(3)		
#define GPIO_IN_REG		(3)		
#define NUM_AUX_REG		(2)		

typedef struct
{
  uint8_t		ADCV[2];									
	uint8_t		ADAX[2];									
	uint8_t 	spiRxBuf[TOTAL_IC*BYTES_IN_REG+CMD_LEN];		
	uint8_t	 	spiTxBuf[TOTAL_IC*BYTES_IN_REG+CMD_LEN];		
	uint8_t		chipConfig[TOTAL_IC][BYTES_IN_REG];	   					
	uint16_t 	auxVolts[TOTAL_IC][6];	   //mv			
	uint16_t	cellVolts[TOTAL_IC][12];   //mv	
}LTC6804_handle;
...
typedef struct {
	...
	uint8_t 	ADCOPT;	// ADC mode group selection
...
//硬件时钟及IO
#define LTC6804SPI                             SPI2
#define LTC6804SPI_CLK_ENABLE()                __HAL_RCC_SPI2_CLK_ENABLE()
#define LTC6804SPI_GPIO_CLK_ENABLE()           __HAL_RCC_GPIOB_CLK_ENABLE()
#define LTC6804SPI_GPIO_PORT                   GPIOB
#define LTC6804SPI_CS_PIN                      GPIO_PIN_12
#define LTC6804SPI_SCK_PIN                     GPIO_PIN_13
#define LTC6804SPI_MISO_PIN                    GPIO_PIN_14
#define LTC6804SPI_MOSI_PIN                    GPIO_PIN_15
#define LTC6804SPI_PIN_AF                      GPIO_AF5_SPI2
...
//SPI总线CS控制
#define LTC6804SPI_CS_LOW() (HAL_GPIO_WritePin(LTC6804SPI_GPIO_PORT,LTC6804SPI_CS_PIN ,GPIO_PIN_RESET))
#define LTC6804SPI_CS_HIGH() (HAL_GPIO_WritePin(LTC6804SPI_GPIO_PORT,LTC6804SPI_CS_PIN ,GPIO_PIN_SET))
...
extern SPI_HandleTypeDef  g_LTC6804SPIHandle;
...
//每次读取单体电压和GPIO口电压需要写一次配置寄存器，否则VREG1 VREG2均不能正常上电
